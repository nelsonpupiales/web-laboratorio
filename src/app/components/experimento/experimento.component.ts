  ...
  await this.dataApi.cargarUnExperimento(id)
    .subscribe(
      experimento => {
        ...
        this.variExperimento=this.experimento.id;
        this.experimento['idMateria'] = this.route.snapshot.params["idMateria"];
        this.experimento.id =  this.route.snapshot.params["id"];

        console.log("Id Experimento", this.variExperimento)
  ...
  this.dataApi.cargaDatosPracticas(idEstudiante)
    .subscribe(
      practica => {
        ...
        let idEx = this.route.snapshot.params["id"];
        console.log("Carga de Id Experimento --> ",idEx)

        const id = this.route.snapshot.params["idExperimento"];
        let longitud = (Object.keys(practica).length);

        //let idEX = "60bbe0bc9f239c0272071957"
        console.log(longitud);
        for (let i = 0; i < longitud; i++) {
          //console.log("Fuera de If --> ", practica)
